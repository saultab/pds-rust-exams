...
impl<K: Hash + Eq + PartialEq + PartialOrd + Clone + Send, V: Clone + Send> Joiner<K,V>{
    ...
    fn supply(&self, key: K, value: V) -> HashMap<K,V>{
        ...
        if (*lock).0.len() == self.n_threads {self.cv.notify_all()} //last to insert into map allows the map copy process to begin for all threads
        ...
        if (*lock).1 == self.n_threads { //last thread to clone the map into ret clears the map
            (*lock).1 = 0;
            (*lock).0.clear();
